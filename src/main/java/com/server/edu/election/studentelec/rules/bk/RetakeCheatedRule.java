...
public class RetakeCheatedRule extends AbstractRuleExceutor {
	...
	public void prepare() {
		...
		//
		// Set<RuleConfigParam> params =
		// getParams(context.getState().getProfile(entityDao).getElectConfigs());
		//
		...
		// violationParam =
		// uniqueParam(context.getState().getProfile(entityDao).getElectConfigs());
		// }
		//
		// if(null != violationParam &&
		// StringUtils.isNotBlank(violationParam.getValue())){
		// examStatusNames =
		// StringUtils.split(StringUtils.trim(violationParam.getValue()),",");
		// }
		// Map<Long,Course> cheatedCourses = CollectUtils.newHashMap();
		// if(null != examStatusNames){
		// OqlBuilder<Course> builder = OqlBuilder.from(CourseGrade.class.getName() + "
		// courseGrade");
		...
		// builder.where("examGrade.examStatus.name in(:violationNames)",
		// examStatusNames);
		// if(semesterParam != null) {
		// Semester prevSemester =
		// semesterService.getPrevSemester(context.getState().getSemester(entityDao));
		// if(prevSemester != null) {
		// builder.where("courseGrade.semester = :prevSemester", prevSemester);
		// }
		// }
		// Set<Course> cheatedCourseSet =
		// CollectUtils.newHashSet(entityDao.search(builder));
		// for (Course course : cheatedCourseSet) {
		// for (ElectCourseSubstitution courseSubstitution :
		// context.getState().getCourseSubstitutions()) {
		...
		// }
		//
		// context.getState().getParams().put(STATE_PARAM_CHECTED_COURSES,
		// cheatedCourses);
		// context.getState().getParams().put(STATE_PARAM_CHECTED_SUB_COURSES,
		// cheatedCourseSubstitutionIds);
	...
	public boolean checkRule(ElecContext context, TeachingClassCache courseClass) {
		// Map<Long,Course> cheatedCourses = (Map<Long, Course>)
		// state.getParams().get(STATE_PARAM_CHECTED_COURSES);
		// Set<Long> cheatedCourseSubstitutionIds = (Set <Long>)
		// state.getParams().get(STATE_PARAM_CHECTED_SUB_COURSES);
		// return !cheatedCourses.containsKey(lesson.getCourse().getId()) &&
		// !cheatedCourseSubstitutionIds.contains(lesson.getCourse().getId());
		// if(!result){
		// context.addMessage(new
		// ElectMessage("有违纪记录,不能重修",ElectRuleType.ELECTION,false,context.getLesson()));
		// }
		return false;
	}
