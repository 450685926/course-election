	...
	public ElcResultCountVo elcResultCount(PageCondition<ElcResultQuery> page) {
		...
		if(condition.getDimension().intValue() == Constants.ONE){
			...
			for (ElcResultDto elcResultDto : elcResultList) {
				...
				elcResultDto.setNumberOfelectedPersonsPoint(elcResultDto.getStudentNum().intValue()==0?new BigDecimal(0):new BigDecimal(numberOfelectedPersons).divide(new BigDecimal(elcResultDto.getStudentNum()),2,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(100)));
