...
public class ReportManagementServiceImpl implements ReportManagementService {
     ...
     */
    @Override
    public ExcelResult exportStudent(Long calendarId, String studentCode) {
        ExcelResult excelResult = ExportExcelUtils.submitTask("studentTable", new ExcelExecuter() {
            @Override
            public GeneralExcelDesigner getExcelDesigner() {
                ExcelResult result = this.getResult();
                List<StudnetTimeTable> list = findStudentTimetab(calendarId, studentCode);
                Double count = list.size() / 1.5;
                result.setDoneCount(count.intValue());
                this.updateResult(result);
