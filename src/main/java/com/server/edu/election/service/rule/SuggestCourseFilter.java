 ...
 */
public class SuggestCourseFilter extends AbstractElectableLessonFilter
{
...
//      if(!result) {
        //          context.addMessage(new ElectMessage("请按照建议课表选课", ElectRuleType.ELECTION, false, context.getLesson()));
        //      }
        return false;
    }
     ...
     * @return
     */
    public boolean dirtyWork(TeachingClass lesson, ElectState state)
    {
        //		// 如果是重修，且重修不检查教学班，那么学生肯定能选这个课
        //		if(retakeService.isRetakeCourse(state, lesson.getCourse().getId())  && !retakeService.isCheckTeachClass(state.getProfile(entityDao).getElectConfigs())){
        //			return true;
        //		}
